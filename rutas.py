...
def habitacionesAdmin():
    ...
    return render_template("habitacionesAdmin.html", 
    form1 = formulario1, 
    form2 = formulario2, 
    form3 = formulario3, 
    data1 = [{'capacidad': 'Individual'}, {'capacidad': '2 personas'}, {'capacidad': '4 personas'}], 
...
# CAMBIOS REALIZADOS
@app.route('/realizarreservaadmin', methods=['GET', 'POST'])
def realizarReservaAdmin():
    formulario1 = RealizarReservaAdmin()
    estandar_path = os.path.join(app.config['UPLOAD_FOLDER'], 'estandarReservaAdmin.png')
    ...
    return render_template("realizarReservaAdmin.html", titulo = "Realizar Reserva como Administrador", logo_img = logo_path, form1 = formulario1, estandar = estandar_path)

@app.route('/editarreservaadmin', methods=['GET', 'POST'])
def editarReservaAdmin():
    formulario1 = EditarReservaAdmin()

    if request.method== "POST":
        if request.form['button'] == 'Editar Reserva':
            sql_edit_reserva(formulario1.numId.data, formulario1.fechaInicio.data, formulario1.fechaFinal.data, formulario1.numHabitacion.data)
            reserva = sql_select_reserva(formulario1.numId.data)
            formulario1.codigoReserva.data = reserva[0][0]

    return render_template("editarReservaAdmin.html", form1 = formulario1, titulo = "Gestión Reservas", logo_img = logo_path)

@app.route('/eliminarreservaadmin', methods=['GET', 'POST'])
def eliminarReservaAdmin():
    formulario1 = EliminarReservaAdmin()
    estandar_path = os.path.join(app.config['UPLOAD_FOLDER'], 'estandarReservaUser.png')

    if request.method== "POST":
        if request.form1['button'] == 'Eliminar Reserva':
            sql_delete_reserva(formulario1.codigoReserva.data)

    return render_template("eliminarReservaAdmin.html", titulo = "Eliminar Reserva como Administrador", logo_img = logo_path, form1 = formulario1, estandar = estandar_path)
